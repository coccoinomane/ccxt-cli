...
export async function currencies(exchangeId: string) {
    ...
    const table = new Table({
        head: [chalk.cyan('ID'), chalk.cyan('Code'), chalk.cyan('Precision'), chalk.cyan('Name'), chalk.cyan('Deposit'), chalk.cyan('Withdraw'), chalk.cyan('Networks')],
    });

    Object.keys(exchange.currencies).forEach((key) => {
        const currency = exchange.currencies[key];
        table.push([key, currency.code, currency.precision, currency.name, currency.deposit, currency.withdraw, Object.keys(currency.networks).join(', ')]);
    });
