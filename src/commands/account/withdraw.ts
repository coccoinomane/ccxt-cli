 ...
 */
export async function withdraw(exchangeId: string, currency: string, amount: string, address: string, options: WithdrawOptions = {}) {
    try {
        ...
        const confirmed = await confirmAction(`Are you sure you want to withdraw ${amount} ${currency} to ${address}?`, options.force);
        ...
        // Execute withdrawal
        const result = await exchange.withdraw(currency.toUpperCase(), parseFloat(amount), address, options.tag);
