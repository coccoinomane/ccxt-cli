...
export function formatBalance(balance: any) {
    ...
    const table = new Table({
        head: [chalk.cyan('Currency'), chalk.cyan('Total'), chalk.cyan('Available'), chalk.cyan('In Orders')],
    });
    ...
    nonZeroBalances.forEach(([currency, total]) => {
        ...
        table.push([currency, parseFloat(total as any).toFixed(8), parseFloat(free as any).toFixed(8), used > 0 ? parseFloat(used as any).toFixed(8) : '0.00000000']);
    });
